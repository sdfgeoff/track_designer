...
pub fn sort_vertex_group_radial(vertices: &Vec<Vertex>, vert_group: &mut VertexGroup) {
    ...
    vert_group.push(
        vert_group
            .first()
            .expect("Tried to sort empty vertex group")
            .clone(),
...
pub fn offset_vert_group(group: &VertexGroup, offset: u32) -> VertexGroup {
    group
        .iter()
        .map(|x| VertWeight::new(x.vert_index + offset, x.weight))
        .collect()
}
...
impl Mesh {
    ...
    pub fn extend(&mut self, other: &Self) -> u32 {
        ...
        for (name, vertex_group) in other.vertex_groups.iter() {
            match self.vertex_groups.get_mut(name) {
                ...
                None => {
                    self.vertex_groups.insert(
                        name.to_string(),
                        offset_vert_group(vertex_group, vert_offset),
                    );
                }
            }
        }
    ...
    pub fn merge_by_distance(&self, distance: f32) -> Self {
        ...
        for (index, vert) in self.vertices.iter().enumerate() {
            ...
            if !merged {
                ...
                index_remap.insert(
                    ...
                    (new_vertices.len() - 1) as VertexIndex,
                ); //.expect_none("Overwriting vert");
            }
        }
        ...
        for face in self.faces.iter() {
            let v1 = index_remap
                .get(&(face.0 as VertexIndex))
                .expect("Index remap missing vertex")
                .clone();
            let v2 = index_remap
                .get(&(face.1 as VertexIndex))
                .expect("Index remap missing vertex")
                .clone();
            let v3 = index_remap
                .get(&(face.2 as VertexIndex))
                .expect("Index remap missing vertex")
                .clone();
