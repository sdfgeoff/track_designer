...
pub fn generate_track(track_description: TrackDescription, shape: TrackShapeDescription) -> Mesh {
    ...
    let mut outer_surface = track_description
        .outer_surface
        .get_mesh(track_description.belt_width_mm, outer_segment_length_mm);
    outer_surface.linear_offset(Vec3::new(
        0.0,
        0.0,
        track_description.belt_thickness_mm / 2.0,
    ));
    let mut inner_surface = track_description
        .inner_surface
        .get_mesh(track_description.belt_width_mm, inner_segment_length_mm);
    inner_surface.linear_offset(Vec3::new(
        0.0,
        0.0,
        -track_description.belt_thickness_mm / 2.0,
    ));
    ...
    match shape {
        ...
        TrackShapeDescription::Path(path) => unimplemented!(),
    }

    sort_vertex_group_radial(
        &outer_surface.vertices,
        outer_surface.vertex_groups.get_mut("edge_right").unwrap(),
    );
    sort_vertex_group_radial(
        &outer_surface.vertices,
        outer_surface.vertex_groups.get_mut("edge_left").unwrap(),
    );
    sort_vertex_group_radial(
        &inner_surface.vertices,
        inner_surface.vertex_groups.get_mut("edge_right").unwrap(),
    );
    sort_vertex_group_radial(
        &inner_surface.vertices,
        inner_surface.vertex_groups.get_mut("edge_left").unwrap(),
    );
    ...
    let track_sidewall_right = generate_vertex_bridge(
        ...
        outer_surface
            ...
            .get("edge_right")
            .as_ref()
            .expect("Track outer Surface missing edge_right vertex group"),
        inner_surface
            .vertex_groups
            .get("edge_right")
            .as_ref()
            .expect("Track inner Surface missing edge_left vertex group"),
    );
    let track_sidewall_left = generate_vertex_bridge(
        ...
        outer_surface
            ...
            .get("edge_left")
            .as_ref()
            .expect("Track outer Surface missing edge_left vertex group"),
    );
