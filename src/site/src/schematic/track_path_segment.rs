...
impl TrackPathSegmentDrawing {
    ...
    pub fn update_from_track_path_segment(&mut self, segment: &TrackPathSegment) -> Result<(), JsValue> {
        self.line.set_attribute("x1", &format!("{}", segment.line.start[0]))?;
        self.line.set_attribute("y1", &format!("{}", segment.line.start[1]))?;
        self.line.set_attribute("x2", &format!("{}", segment.line.end[0]))?;
        self.line.set_attribute("y2", &format!("{}", segment.line.end[1]))?;
        ...
        let large_arc = match (segment.arc.end_angle - segment.arc.start_angle) >= std::f32::consts::PI {
            true => "1",
            false => "0"
        };
    ...
    pub fn delete(&self) -> Result<(), JsValue> {
        self.line.parent_node().expect("Not in drawing!!!").remove_child(&self.line)?;
        self.arc.parent_node().expect("Not in drawing!!!").remove_child(&self.arc)?;
        Ok(())
    }
}
