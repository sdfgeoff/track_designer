...
impl App {
    pub fn new(canvas: HtmlCanvasElement) -> Self {
        ...
        renderer
            .add_stl(&stl_data)
            .expect("failed to load stl to renderer");
    ...
    pub fn mouse_move(&mut self, event: MouseEvent) {
        ...
        match self.click_location {
            Some(location) => {
                ...
                let percentage_x =
                    (delta.0 as f32) / (self.renderer.resolution.0 as f32) * DRAG_SENSITIVITY;
                let percentage_y =
                    (delta.1 as f32) / (self.renderer.resolution.0 as f32) * DRAG_SENSITIVITY;
